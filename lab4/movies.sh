...
function query_year () {
    ...
    local RESULTS_LIST=()
    for MOVIE_FILE in ${MOVIES_LIST}; do
        if grep "| Year"  ${MOVIE_FILE} | grep -q "${YEAR}"; then
            RESULTS_LIST+=( "${MOVIE_FILE}" )
        fi
    done
    echo "${RESULTS_LIST[@]:-}"
...
while getopts ":hd:t:a:f:x:k:f:" OPT; do
    case ${OPT} in
        h)
            print_help
            exit 0
...
echo "Nie podano parametru -d"
fi

if [[ ${CHECK_TXT:-false} ]]; then 
    if [[! ${QUERY_FILE}==${*.txt} ]]; then
        mv ${QUERY_FILE} ${QUERY_FILE}.txt
    fi    
fi
